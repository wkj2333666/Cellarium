...
impl GrowthScene {
    pub fn from_editor(editor: &GrowthEditorState) -> Self {
        Self { plot: editor.plot().data.clone(), stale: editor.plot().stale, cursor: None }
    }

    pub fn sample_at_pixel(&self, x: u32, width: u32) -> Option<usize> {
        if self.plot.is_empty() || width == 0 { return None; }
        Some(((x as usize * self.plot.len().saturating_sub(1)) / width as usize).min(self.plot.len() - 1))
    }
    ...
    pub fn select_pixel(&mut self, x: u32, width: u32) -> Option<GrowthCursor> {
        ...
        let cursor = GrowthCursor { sample, input: sample as f32 / self.plot.len().saturating_sub(1).max(1) as f32, value };
...
impl GraphicsScene for GrowthScene {
    fn render_rgba(&self, width: u32, height: u32) -> GraphicsFrame {
        ...
        draw_line(&mut rgba, width, height, (24, height as i32 - 24), (width as i32 - 8, height as i32 - 24), axis);
        draw_line(&mut rgba, width, height, (24, 8), (24, height as i32 - 24), axis);
        let valid = self.plot.iter().filter_map(|v| *v).collect::<Vec<_>>();
        let (min, max) = valid.iter().copied().fold((0.0_f32, 1.0_f32), |(lo, hi), value| (lo.min(value), hi.max(value)));
        let span = (max - min).max(1e-6);
        let mut previous = None;
        for (index, value) in self.plot.iter().enumerate() {
            let Some(value) = value else { previous = None; continue; };
            let x = 24 + (index * (width.saturating_sub(33) as usize) / self.plot.len().saturating_sub(1).max(1)) as i32;
            let y = height as i32 - 24 - (((value - min) / span) * (height.saturating_sub(33) as f32)) as i32;
            let point = (x, y);
            if let Some(prev) = previous { draw_line(&mut rgba, width, height, prev, point, [80, 220, 140, 255]); }
            previous = Some(point);
        }
        if let Some(cursor) = self.cursor {
            if !self.plot.is_empty() {
                let x = 24 + (cursor.sample * width.saturating_sub(33) as usize / self.plot.len().saturating_sub(1).max(1)) as i32;
        ...
        if self.stale {
            for x in 0..width { blend(&mut rgba, width, height, x as i32, 0, [220, 90, 90, 255]); }
        }
        GraphicsFrame::new(width, height, rgba, 0).expect("valid growth frame")
    }
}

fn draw_line(rgba: &mut [u8], width: u32, height: u32, start: (i32, i32), end: (i32, i32), color: [u8; 4]) {
    let mut x = start.0; let mut y = start.1;
    let dx = (end.0 - x).abs(); let sx = if x < end.0 { 1 } else { -1 };
    let dy = -(end.1 - y).abs(); let sy = if y < end.1 { 1 } else { -1 };
    let mut err = dx + dy;
    loop {
...
mod tests {
    ...
    fn cursor_maps_to_curve_sample() {
        let editor = GrowthEditorState::new("inner * inner", ExternalSymbols::new(&["inner"], &[]), BTreeMap::new(), "growth");
