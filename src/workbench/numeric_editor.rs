...
impl NumericEditor {
    ...
    pub fn original(&self) -> f64 { self.original }

    pub fn replace(&mut self, source: impl Into<String>) { self.buffer = source.into(); }
    pub fn push(&mut self, character: char) { self.buffer.push(character); }
    pub fn backspace(&mut self) { self.buffer.pop(); }

    pub fn commit(&self) -> Result<f64, NumericError> {
        let value = self.buffer.parse::<f64>().map_err(|_| NumericError::InvalidNumber)?;
    ...
    pub fn cancel(self) -> f64 { self.original }
}
...
mod tests {
    ...
    fn non_finite_and_out_of_range_values_are_rejected() {
        ...
        editor.replace("2.0");
        assert_eq!(editor.commit(), Err(NumericError::OutOfRange { min: -1.0, max: 1.0 }));
    }
}
